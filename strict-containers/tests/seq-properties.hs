...
import Data.Strict.Sequence.Autogen.Internal
  ( Sized (..)
  , Seq (Seq)
  , FingerTree(..)
  , Node(..)
...
prop_lazyfoldr' xs =
    ...
    foldr'
        (\e _ ->
              e)
        (error "Data.Strict.Sequence.Autogen.foldr': should be lazy in initial accumulator")
        xs ===
    ()
...
prop_lazyfoldl' xs =
    ...
    foldl'
        (\_ e ->
              e)
        (error "Data.Strict.Sequence.Autogen.foldl': should be lazy in initial accumulator")
        xs ===
    ()
