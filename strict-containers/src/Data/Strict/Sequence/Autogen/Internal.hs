...
-----------------------------------------------------------------------------
-- |
-- Module      :  Data.Strict.Sequence.Autogen.Internal
...
import Data.Strict.ContainersUtils.Autogen.Coercions ((.#), (.^#))
import Data.Coerce
import qualified GHC.Exts

import Data.Functor.Identity (Identity(..))

import Data.Strict.ContainersUtils.Autogen.StrictPair (StrictPair (..), toPair)
...
mfixSeq f = fromFunction (length (f err)) (\k -> fix (\xk -> f xk `index` k))
  where
    err = error "mfix for Data.Strict.Sequence.Autogen.Seq applied to strict function"
...
-- is a generalization of many Data.Strict.Sequence.Autogen methods.
...
index (Seq xs) i
  ...
  | fromIntegral i < (fromIntegral (size xs) :: Word) = case lookupTree i xs of
                Place _ (Elem x) -> x
  | otherwise   =
      error $ "index out of bounds in call to: Data.Strict.Sequence.Autogen.index " ++ show i
...
-- @since 0.5.6.2
fromFunction :: Int -> (Int -> a) -> Seq a
fromFunction len f | len < 0 = error "Data.Strict.Sequence.Autogen.fromFunction called with negative len"
                   | len == 0 = empty
...
-- https://github.com/haskell/containers/blob/74034b3244fa4817c7bef1202e639b887a975d9e/Data.Strict.Sequence.Autogen.hs#L3532
--
-- Given a list
--
-- [1..302]
...
zipWith' f s1 s2 = splitMap uncheckedSplitAt goLeaf s2 s1
  where
    ...
    goLeaf _ _ = error "Data.Strict.Sequence.Autogen.zipWith'.goLeaf internal error: not a singleton"
