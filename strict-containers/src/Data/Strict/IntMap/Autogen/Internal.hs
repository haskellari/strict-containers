...
instance Foldable.Foldable IntMap where
  ...
  {-# INLINABLE elem #-}
  maximum = start
    where start Nil = error "Data.Foldable.maximum (for Data.Strict.IntMap.Autogen): empty map"
          start (Tip _ y) = y
          start (Bin _ m l r)
            | m < 0     = go (start r) l
  ...
  {-# INLINABLE maximum #-}
  minimum = start
    where start Nil = error "Data.Foldable.minimum (for Data.Strict.IntMap.Autogen): empty map"
          start (Tip _ y) = y
          start (Bin _ m l r)
            | m < 0     = go (start r) l
            | otherwise = go (start l) r
...
intMapDataType = mkDataType "Data.Strict.IntMap.Autogen.Internal.IntMap" [fromListConstr]
